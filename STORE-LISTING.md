...
GİZLİLİK
Hiçbir veri toplanmaz, hiçbir yere gönderilmez. Sunucu yoktur. Tüm işlem
tarayıcınızda gerçekleşir. Yalnızca sahibinden.com'a erişim ister.
...
| Field | Paste this |
|---|---|
| `storage` | Stores a 24-hour cache of per-listing verdicts so the same listing is not requested twice, plus two user preferences (on/off, badge visibility). No browsing data. |
...
**Data usage disclosures** — tick **none** of the collection categories, then
...
**Privacy policy URL:** the public URL where you hosted `PRIVACY.md`.
